...
const CreateFlight = () =>{
    const {onSetString,setLoading,setModalTrigger,setMessage,
        activeWorker,flightsSpaces,setFlightSpaces,setModalOpen,logged} = useContext(flightContext);
    const [departureDate,setDepartureDate] = useState(new Date());
    const [arrivalDate,setArrivalDate] = useState(new Date());
    const [price,setPrice] = useState("");
    const [plane,setPlane] = useState("");
    const [destination,setDestination] = useState("Adis-Ababa/Ethiopia");
    const [seatNumber,setSeatNumber] = useState("");
    const [airport,setAirport] = useState("Yaounde/Nsimalen");
    const [category,setCategory] = useState("Business");

    const airportList = ["Yaounde/Nsimalen","Douala","Garoua"];
    const destinationPort = ["Adis-Ababa/Ethiopia","Lyon/France","Bruxells/Belgium"];
    ...
    const onCreateFlight = () =>{
        ...
        //run save function
        let data = flightCreationHandler(timeObj,flightObj,workerObj,setLoading,errorObj);
